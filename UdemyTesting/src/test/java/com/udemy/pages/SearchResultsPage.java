...
public class SearchResultsPage extends BasePage {
    ...
    // Search results container - More robust locators
    ...
    // Alternative course card selectors (Udemy uses different layouts)
    ...
    public void waitForResultsToLoad() {
        waitForPageLoad();
        sleep(3000); // Wait for dynamic content to load
        handleCookiePopup();
    }
    ...
    public List<String> getAllCourseTitles() {
        ...
        try {
            ...
            for (WebElement title : titleElements) {
                ...
                if (!text.isEmpty() && text.length() > 5) { // Filter out short text
                    titles.add(text);
                }
            }
        } catch (Exception e) {
    ...
    public boolean isNoResultsDisplayed() {
        try {
            ...
            // Also check if course count is 0
    ...
    public boolean isAppropriateFeedbackDisplayed() {
        // For negative tests, appropriate feedback means:
        // 1. We're on a search page, OR
        // 2. No results message is shown, OR
        // 3. The page shows some form of response (even suggestions)
