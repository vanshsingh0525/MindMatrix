...
public class SearchResultsPage extends BasePage {
    ...
    /**
     * Apply features filter (subtitles, quizzes, etc.).
     */
    ...
    public void clearAllFilters() {
        try {
            ...
            // Navigate to base search URL to clear filters
    ...
    /**
     * Check if filter is applied (check URL or UI).
     */
    ...
    // ==================== COURSE CARD DETAIL METHODS ====================

    /**
     * Get all course ratings from displayed cards.
     */
    ...
    /**
     * Check if all displayed courses meet minimum rating.
     */
    public boolean allCoursesHaveMinimumRating(double minRating) {
        List<Double> ratings = getAllCourseRatings();
        if (ratings.isEmpty()) {
            return true; // If no ratings found, don't fail the test
        }
    ...
    /**
     * Get all course prices from displayed cards.
     */
    ...
    /**
     * Check if all displayed courses are free.
     */
    ...
    /**
     * Check if all displayed courses are paid.
     */
    ...
    /**
     * Get all course levels from displayed cards.
     */
    ...
    private double parseDurationToHours(String durationText) {
        try {
            // Pattern for "X hours" or "X.X hours"
            ...
            // Pattern for "X min" - convert to hours
            ...
            // Return 0 for unparseable durations
        }
        return 0;
    }

    /**
     * Check if course durations are within range.
     */
    ...
    /**
     * Check if any course has bestseller badge.
     */
    ...
    /**
     * Get number of results from header text.
     */
    public int getResultsCountNumber() {
        try {
            ...
            // Return course card count as fallback
        }
        return getNumberOfCourseCards();
    }

    /**
     * Verify that filters affect results (results changed after filter).
     */
    public boolean didFilterAffectResults(int previousCount) {
        int currentCount = getResultsCountNumber();
        // Filter is considered effective if count changed or if results are still present
        return currentCount != previousCount || currentCount > 0;
    }

    /**
     * Get instructor names from course cards.
     */
    ...
    /**
     * Get first course details as map.
     */
